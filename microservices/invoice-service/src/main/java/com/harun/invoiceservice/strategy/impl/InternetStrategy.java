...
import com.harun.invoiceservice.enums.InternetOrganisation;
...
@Component("internetStrategy")
@RequiredArgsConstructor
public class InternetStrategy implements PayStrategy {
    private static final Logger logger = LoggerFactory.getLogger(InternetStrategy.class);
    private final PaymentServiceClientImpl paymentServiceClient;

    @Override
    public String pay(Double amount) {
        paymentServiceClient.payInvoice(amount);
        String message = StringBuilderUtil.buildMessage(MessageTemplates.BILL_PAID, "internet", amount, InternetOrganisation.TTNET.name());
        logger.info(message);
